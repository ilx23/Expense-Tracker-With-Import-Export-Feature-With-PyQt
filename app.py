...
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QListWidget, QPushButton, \
    QDialog, QLabel, QLineEdit, QHBoxLayout, QMessageBox, QComboBox, QDateEdit, QFileDialog, QListWidgetItem
...
class ExpenseTracker(QWidget):
    ...
    def initUI(self):
        ...
        # Buttons for adding, editing, deleting, and clearing expenses
        ...
        self.delete_all_expense_button = QPushButton('Delete All')

        # Export and Import buttons
        self.export_expense_button = QPushButton('Export')
        self.import_expense_button = QPushButton('Import')
        ...
        button_layout.addWidget(self.delete_all_expense_button)
        layout.addLayout(button_layout)

        exp_imp_button_layout = QHBoxLayout()
        exp_imp_button_layout.addWidget(self.export_expense_button)
        exp_imp_button_layout.addWidget(self.import_expense_button)
        layout.addLayout(exp_imp_button_layout)
        ...
        self.delete_all_expense_button.clicked.connect(lambda: self.expense_list.clear())
        self.export_expense_button.clicked.connect(self.export_expense)
        self.import_expense_button.clicked.connect(self.import_expense)
    ...
    # Method to export expenses to a CSV file
    def export_expense(self):
        if self.expense_list.count() == 0:
            QMessageBox.warning(self, "Error", "There are no items in your list. Please add some expenses first.")
        else:
            filename, _ = QFileDialog.getSaveFileName(self, "Save File", "", "CSV Files (*.csv)")
            if filename:
                expenses = []
                for index in range(self.expense_list.count()):
                    item = self.expense_list.item(index)
                    item_text = item.text()
                    item_in_list = item_text.split("-")
                    expenses.append({
                        "Date": f"{item_in_list[0]}-{item_in_list[1]}-{item_in_list[2]}",
                        "Category": item_in_list[3],
                        "Amount": item_in_list[4]
                    })
                df = pd.DataFrame(expenses)
                df.to_csv(filename, index=False)

    # Method to import expenses from a CSV file
    def import_expense(self):
        ...
        if filename:
            ...
            for _, row in df.iterrows():
                item = QListWidgetItem(f"{row['Date']} - {row['Category']} - {row['Amount']}")
